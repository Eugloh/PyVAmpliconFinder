	...
	foreach my $file (`ls $dir/*.blast`)
	{
		...
		while(<F1>)
		{
			...
			if($kingdom=~/Viruses/i && $sname=~/polyoma/i){
				...
				#~ if($percid>=90.0){
				if($percid>=85.0){							#Test thershold 15%
...
my $fac = Bio::Tools::Run::StandAloneBlastPlus->new(
   ...
   -db_data => $dirname.'/databases/PyV/References_PyV.fasta',
   -create => 1,
   -alphabet=>'dna'
		);
...
sub treatputative{
	...
	if($boolean_infofile eq "true"){
		...
		print T "VIRUSname\t%dissimilarity\tAbundance\tN°reads\tGInum\tAlignmentPosition_MegaBlast\tVIRUS_closest_MegaBlast\tPool\tTissu\tPrimer\tLength\tAlignmentPositionBlastN_start:stop(length)\tVIRUS_closest_Blast\tBlastN_Classification\tRaxML_closest_PyV\tRaxML_Classification\tSequence(s)\n";
	}
	else{
		print T "VIRUSname\t%dissimilarity\tAbundance\tN°reads\tGInum\tAlignmentPosition_MegaBlast\tVIRUS_closest_MegaBlast\tPool\tLength\tAlignmentPositionBlastN_start:stop(length)\tVIRUS_closest_Blast\tBlastN_Classification\tRaxML_closest_PyV\tRaxML_Classification\tSequence(s)\n";
	}
	...
	while(<IN>){
		...
		if($querytax{$idselected}=~/^(\S+)polyomavirus (\d+)/){
			...
			if($1=~/alpha/i or $1=~/beta/i or $1=~/gamma/i or $1=~/mu/i or $1=~/nu/i or $1=~/delta/i){
				$genus="Unreferenced";
			}
			else{
				$genus=$1;
