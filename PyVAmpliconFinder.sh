...
PyVAmpliconFinder development, based on PyVAmpliconFinder tools (Robitaille).


$(basename "$0") [-h] [-t threads] [-b \"nt\" database] [-f info_file] [-i identity thershold] -s fastq_files_suffix -d input_dir -o output_dir -- program to process amplicon-based NGS data
Version 1.0
...
then
	...
	##	Dereplication
	echo -e "~~	Dereplicate	~~";
	find . -name "${suffix}*_good.fasta" | xargs --max-args=1 --max-procs=${threads} -- bash -c 'vsearch --quiet --derep_fulllength '${tmpdir}/'${0} --sizeout --threads '${threads}' --relabel_sha1 --fasta_width 0 --strand both --output '${tmpdir}'/$(basename "${0/_good.fasta/}")_lin_der.fasta --log $(basename "${0/_good.fasta/}").log  &>> '${logfile}';'
